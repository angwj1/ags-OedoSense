"""
Create a software (with GUI) named "AGS OedoSense" to:
...
def settings_window(settings):
    ...
    layout = [[sg.T("SETTINGS")],
              ...
              [sg.Checkbox("Casagrande's Method (1936)", settings["Param"]["print_casagrande"], key="-PRINT_CASAGRANDE-")],
              [sg.Checkbox("Maximum Curvature Method (Gregory et al., 2006)", settings["Param"]["print_max_curvature"], key="-PRINT_MAXIMUM_CURVATURE-")],
              [sg.Checkbox("Oikawa's Method (1987)", settings["Param"]["print_oikawa"], key="-PRINT_OIKAWA-")],
    ...
    while True:
        ...
        if event == "Help":
            ...
            sg.popup(window_title, "Instructions",
                "1. Select the input folder containing the MS excel files (in AGS format) \n2. Select the output folder that will store the results generated by the application \n3. Adjust the settings as necessary \n4. Check the data format \n5. Generate graphs and calculate preconsolidation pressure \n", 
                grab_anywhere=True)
            window.reappear()
        if event == "About":
            window.disappear()
            sg.popup(window_title, "Version 2.0 - Built by Ang Wei Jian, 2023", 
                "AGS OedoSense makes sense of oedometer data in AGS files. It reads oedometer test data directly from MS Excel files in AGS format, checks the data format and determines the preconsolidation pressure with three methods: \n1. Casagrande's Method (1936) \n2. Maximum Curvature Method (Gregory et al., 2006) \n3. Oikawa's Method (1987)\n", 
                grab_anywhere=True)
            window.reappear()
        if event == "Settings":

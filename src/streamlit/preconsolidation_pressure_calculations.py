...
def calculate_pc_Casagrande(df, title, x='CONS_INCF', y='CONS_INCE', const='CONG_PRCP',
    ...
    if printer==True: 
        ...
        fig_name_ca = title + '.pdf'
    
        # Save the figure as bytes in local memory via BytesIO
        # At this point, the pointer is at the end of the stream.
        output_fig_ca = BytesIO()
        fig.savefig(output_fig_ca, format="pdf")
        
        # Reset the pointer to the start so that it can read all the binary content
        output_fig_ca.seek(0)
        plt.close(fig)

    return pc_ca, err_ca, output_fig_ca, fig_name_ca
...
def calculate_pc_Oikawa(df, title, x='CONS_INCF', y='CONS_INCE', const='CONG_PRCP',
               label_x=r"Effective Vertical Stress, $\sigma_v'$  [kPa]",
    ...
    if printer==True: 
        ...
        fig.tight_layout()  # otherwise the right y-label is slightly clipped
        fig_name_oi = title + '.pdf'
    
        # Save the figure as bytes in local memory via BytesIO
        # At this point, the pointer is at the end of the stream.
        output_fig_oi = BytesIO()
        fig.savefig(output_fig_oi, format="pdf")
        
        # Reset the pointer to the start so that it can read all the binary content
        output_fig_oi.seek(0)
        plt.close(fig)

    return pc_oi, err_oi, output_fig_oi, fig_name_oi
...
def calculate_pc_MC(df, title, x='CONS_INCF', y='CONS_INCE', const='CONG_PRCP',
               label_x=r"Effective Vertical Stress, $\sigma_v'$  [kPa]",
               label_y='Void Ratio, e [-]', printer=True, troubleshoot_mode=False):
    ...
    if printer==True: 
        ...
        fig.tight_layout()  # otherwise the right y-label is slightly clipped
        fig_name_mc = title + '.pdf'
    
        # Save the figure as bytes in local memory via BytesIO
        # At this point, the pointer is at the end of the stream.
        output_fig_mc = BytesIO()
        fig.savefig(output_fig_mc, format="pdf")
        
        # Reset the pointer to the start so that it can read all the binary content
        output_fig_mc.seek(0)
        plt.close(fig)

    return pc_mc, err_mc, output_fig_mc, fig_name_mc
